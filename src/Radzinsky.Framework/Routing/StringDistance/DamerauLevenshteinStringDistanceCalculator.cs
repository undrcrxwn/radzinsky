...
public class DamerauLevenshteinStringDistanceCalculator
{
    public float CalculateDistancePerCharacter(ReadOnlySpan<char> a, ReadOnlySpan<char> b)
    {
        var distance = CalculateDistance(a, b);
        var averageStringLength = (a.Length + b.Length) / 2f;
        return distance / averageStringLength;
    }
    
    public int CalculateDistance(ReadOnlySpan<char> a, ReadOnlySpan<char> b)
    {
        var bounds = (Height: a.Length + 1, Width: b.Length + 1);
        ...
        for (var width = 1; width < bounds.Width; width++)
        {
            ...
            if (height > 1 &&
                width > 1 &&
                a[height - 1] == b[width - 2] &&
                a[height - 2] == b[width - 1])
